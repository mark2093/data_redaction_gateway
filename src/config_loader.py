"""
Configuration loader and manager for the redaction gateway.
...
class ConfigLoader:
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        if self.app_config.environment == 'production':
            if 'default' in self.app_config.security.hmac_secret:
                errors.append("Using default HMAC secret in production")
            
            if 'default' in self.app_config.security.encryption_key:
                errors.append("Using default encryption key in production")
            
            if 'dev-api-key' in str(self.app_config.security.api_keys):
                errors.append("Using default API keys in production")
            
            if not self.app_config.security.tls_enabled:
                warnings.append("TLS not enabled in production")
        
        # Check policy file exists
        policy_path = Path(self.app_config.redaction.policy_file)
        if not policy_path.exists():
            errors.append(f"Policy file not found: {policy_path}")
        
        # Check cache settings
        if self.app_config.cache.enabled:
            if self.app_config.cache.ttl_seconds <= 0:
                warnings.append("Cache TTL should be positive")
            
            if self.app_config.cache.max_size <= 0:
        ...
        return {
            ...
            'policy_file': self.app_config.redaction.policy_file
        }
