...
def serve(host, port, reload):
    ...
    host = host or app_config.server.host
    port = port or app_config.server.port
    
    console.print(f"[green]Starting {app_config.name} v{app_config.version}...[/green]")
    console.print(f"[blue]Server: http://{host}:{port}[/blue]")
    console.print(f"[yellow]Environment: {app_config.environment}[/yellow]")
    ...
    uvicorn.run(
        ...
        port=port,
        reload=reload,
        workers=1 if reload else app_config.server.workers
    )
...
def redact(input_file, api_key, url):
    ...
    # Use config defaults if not provided
    api_key = api_key or (app_config.security.api_keys[0] if app_config.security.api_keys else 'dev-api-key-12345')
    url = url or f"http://{app_config.server.host}:{app_config.server.port}"
...
def metrics(url, api_key):
    ...
    async def _get_metrics():
        headers = {app_config.security.api_key_header_name: api_key}
